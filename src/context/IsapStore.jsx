...
import { defaultIngestionSecurity, initialIngestionAudit } from '../data/ingestionSecurity.js'
import { createIngestionAudit } from '../services/ingestionSecurityService.js'
...
export function IsapStoreProvider({ children }) {
  ...
  const [companyResearchProfiles, setCompanyResearchProfiles] = useState(() => {
    try {
      const stored = window.localStorage.getItem('isap-company-research-profiles')
      return stored ? JSON.parse(stored) : {}
    } catch {
      return {}
    }
  ...
  const [ingestionSecurity, setIngestionSecurity] = useState(() => {
    try { const stored = window.localStorage.getItem('isap-ingestion-security'); return stored ? JSON.parse(stored) : defaultIngestionSecurity } catch { return defaultIngestionSecurity }
  })
  const [ingestionAudit, setIngestionAudit] = useState(() => {
    try { const stored = window.localStorage.getItem('isap-ingestion-audit'); return stored ? JSON.parse(stored) : initialIngestionAudit } catch { return initialIngestionAudit }
  })
  ...
  useEffect(() => {
    window.localStorage.setItem('isap-company-research-profiles', JSON.stringify(companyResearchProfiles))
  }, [companyResearchProfiles])

  useEffect(() => { window.localStorage.setItem('isap-ingestion-security', JSON.stringify(ingestionSecurity)) }, [ingestionSecurity])
  useEffect(() => { window.localStorage.setItem('isap-ingestion-audit', JSON.stringify(ingestionAudit)) }, [ingestionAudit])
  ...
  const saveCompanyResearchProfile = useCallback((companyId, profile) => {
    setCompanyResearchProfiles((current) => ({
      ...current,
      [companyId]: { ...profile, companyId, updatedAt: new Date().toISOString() }
    }))
  }, [])

  const addIngestionAudit = useCallback((action, result, detail) => {
    setIngestionAudit((current) => [createIngestionAudit(action, result, detail), ...current].slice(0, 100))
  }, [])

  const updateIngestionSecurity = useCallback((next) => {
    setIngestionSecurity(next)
    setIngestionAudit((current) => [createIngestionAudit('POLICY_UPDATED','PASS','Inbound security policy updated locally.'), ...current].slice(0,100))
  }, [])
