...
export default function CompanyIntelligence({ selectedMeeting, onNavigate }) {
  ...
  const { companySources = [], addCompanySource, deleteCompanySourceById, reanalyseCompanySource, addIngestionAudit, products = [], companyResearchProfiles = {}, saveCompanyResearchProfile } = useIsapStore()
  ...
  const renderTab = () => {
    ...
    if (activeTab === 'sources') return <PublicSourcesPanel companyId={workspace.companyId} companyName={workspace.name} companyIndustry={workspace.industry} sources={companySources} products={products} researchProfile={companyResearchProfiles[workspace.companyId]} onSaveResearchProfile={saveCompanyResearchProfile} onAdd={addCompanySource} onDelete={deleteCompanySourceById} onReanalyse={reanalyseCompanySource} onAudit={addIngestionAudit}/>
