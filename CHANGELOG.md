...
## 3.1 - Secure Ingestion Foundation
- Added inbound security policy, quarantine model, scanner abstraction and security audit foundation.
- Added pre-flight validation laboratory.
- Automatic internet collector remains disabled until a server-side scanner is connected.

## 3.2 - Controlled Public Web Research
- Added real server-side public website research via `/api/public-research`.
- Added SSRF-oriented public-network checks, HTTPS-only policy, redirect validation, same-domain crawling and strict text-only limits.
- Added automatic import of retrieved evidence into Company Intelligence and local Rule Intelligence analysis.
- Added public-research PASS/BLOCK audit events.
- External AI remains disabled; binary downloads remain blocked pending approved malware scanning.

## 3.2.1 - Relevance-first Customer Intelligence
- Reworked Public Sources into an executive research dashboard.
- Added product-aware research ranking using the active ISAP product catalogue.
- Added management discovery from structured JSON-LD and visible leadership text.
- Added rule-based stakeholder ranking and best commercial contact recommendation.
- Added product relevance scores with traceable evidence.
- Added concise key-sales-signal cards and ranked evidence view.
- Raw source text is now collapsed behind an evidence detail control instead of dominating the UI.
- Persisted management research profiles locally per company.
- No external AI introduced; official-domain and HTML/text-only security policy remains active.
